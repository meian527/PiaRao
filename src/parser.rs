...
impl<'a> Parser<'a> {
	...
	fn expect(&mut self, t: lexer::TokenType) {
		if !self.match_ok(t.clone()) {
			self.error(self.cur().l, self.cur().c, &format!("Expected token `{:?}`, but found `{:?}`", t, self.cur().t));
		}
	}
	fn advance(&mut self) {
		self.pos += 1;
	...
	fn consume_or(&mut self, t1: lexer::TokenType, t2: lexer::TokenType) -> bool {
		if self.match_ok(t1.clone()) {
			...
			self.error(self.cur().l, self.cur().c, &format!("Expected token `{:?}` or `{:?}`, but found `{:?}`", t1, t2, self.cur().t));
			false
		}
	}
	...
	fn parse_relational(&mut self) -> ast::Expr {
		...
		while self.match_ok(lexer::TokenType::OpGt) || self.match_ok(lexer::TokenType::OpLt) || self.match_ok(lexer::TokenType::OpGe) || self.match_ok(lexer::TokenType::OpLe) {
			let op = self.cur().v.clone();
			self.advance();
	...
	fn parse_multi(&mut self) -> ast::Expr {
		...
		while self.match_ok(lexer::TokenType::OpMul) || self.match_ok(lexer::TokenType::OpDiv) || self.match_ok(lexer::TokenType::OpMod) {
			let op = self.cur().v.clone();
			self.advance();
	...
	fn parse_factor(&mut self) -> ast::Expr {
		if self.match_ok(lexer::TokenType::Ident) {
			...
			if self.tk_is_primary_start(self.peek(1)) {
				self.advance();
	...
	fn is_primary_start(&mut self) -> bool {
	...
	fn tk_is_primary_start(&self, tk: &lexer::Token) -> bool {
		...
		tk.t == lexer::TokenType::OpSub ||
		tk.t == lexer::TokenType::OpNot ||
	...
	fn parse_primary(&mut self) -> ast::Expr {
		if self.match_ok(lexer::TokenType::Digit) {
		...
		} else if self.match_ok(lexer::TokenType::OpSub) || self.match_ok(lexer::TokenType::OpNot) {
			let op = self.cur().v.clone();
			self.advance();
			ast::Expr::UnaryOp(op, Box::new(self.parse_primary()))
			...
			self.error(self.cur().l, self.cur().c, &format!("Unexpected token `{:?}`", self.cur().t));
			ast::Expr::Null
		}
	}
	...
	fn parse_stmt(&mut self) -> ast::Node {
		...
		let stmt = if self.match_ok(lexer::TokenType::KWLet) {
			self.advance();
			if !self.match_ok(lexer::TokenType::Ident) {
				self.error(line, col, &format!("Expected identifier after 'let'"));
				return ast::Node {
